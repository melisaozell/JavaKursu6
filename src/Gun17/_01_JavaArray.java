...
public class _01_JavaArray
{
    public static void main(String[] args) {
        ...
        for (int i = 0; i < dizi.length; i++) //10 a kadar sayılarda rnd doldur
            dizi[i]=(int)(Math.random()*10);

        for (int i = 0; i < dizi.length; i++) //dizi içeriğini görme
            System.out.println("dizi önce="+dizi[i]);

        for (int i = 0; i < dizi.length; i++) { //çift olanlarda 0,tek olanlarda 1
        ...
        for (int i = 0; i < dizi.length; i++) { //1 ve 0 atama işlemden sonra diziyi yazdır
